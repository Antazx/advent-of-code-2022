...
const commonItems = rucksackList.map(findCommonItem);
const itemValueList = commonItems.map(itemToValue);
const result = itemValueList.reduce((acc, curr) => acc + curr, 0);
console.log('🚀 ~ file: rucksack-reorganization.ts:78 ~ result', result);
